...
<!-- 
If separate preprocessing is required in STATA, include instructions here;
.do files can be included in the /contrib directory.
-->
...
viz = ggplot(df, aes(x=Year, y=value, group=scenario)) +
...
ggplotly(viz, tooltip=c('scenario', 'Year', 'value')) %>%
...
viz = ggplot(df, aes(x=forcats::fct_rev(projection), y=value, fill=region)) +
